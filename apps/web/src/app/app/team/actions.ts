...
export async function createInvitation(formData: FormData) {
  ...
  if (!input.success) redirect("/app/team?error=invalid_invitation");
  ...
  if (error || !data || typeof data.token !== "string") redirect("/app/team?error=invitation_failed");
  redirect(`/app/team?success=invitation_created&invitation=${data.token}`);
}

export async function cancelInvitation(formData: FormData) {
  const id = uuid.safeParse(formData.get("invitation_id"));
  if (!id.success) redirect("/app/team?error=invalid_invitation");
  const supabase = await createClient();
  const { error } = await supabase.rpc("cancel_org_invitation", { invitation_id: id.data });
  if (error) redirect("/app/team?error=invitation_cancel_failed");
  redirect("/app/team?success=invitation_cancelled");
}
...
export async function changeMemberRole(formData: FormData) {
  ...
  if (!input.success) redirect("/app/team?error=invalid_role");
  const supabase = await createClient();
  const { error } = await supabase.rpc("change_org_member_role", { target_user_id: input.data.userId, target_role: input.data.role });
  if (error) redirect("/app/team?error=role_change_failed");
  redirect("/app/team?success=role_changed");
}
...
export async function removeMember(formData: FormData) {
  ...
  if (!input.success) redirect("/app/team?error=invalid_removal");
  const supabase = await createClient();
  ...
  if (error) redirect("/app/team?error=member_removal_failed");
  redirect("/app/team?success=member_removed");
}
