...
export default async function WhatsappSettingsPage({ searchParams }: Props) {
  ...
  return <main className="centered-page"><section className="setup-card whatsapp-settings-card">
    ...
    {connections?.length ? <div className="connection-list">{connections.map((connection) => <article key={connection.id}><div><strong>{connection.display_name}</strong><small>{connection.phone_number || "Telefone aguardando conexão"} · {connection.provider === "whatsapp_cloud" ? "Meta Oficial" : "QR Code"}</small></div><span className={connection.status}>{connection.status === "connected" ? "Conectado" : connection.status === "connecting" ? "Configurando" : connection.status === "qr_code" ? "Aguardando QR" : connection.status === "error" ? "Precisa de atenção" : "Desconectado"}</span><ConnectionActions connectionId={connection.id} provider={connection.provider} status={connection.status} /></article>)}</div> : null}
