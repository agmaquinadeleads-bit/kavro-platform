...
export default async function WhatsappPage({ searchParams }: WhatsappPageProps) {
  ...
  let messages: Array<{ id: string; direction: string; message_type: string; text_content: string | null; status: string; provider_timestamp: string | null; created_at: string; created_by: string | null }> = [];
  ...
  if (selectedConversation) {
    ...
    const response = await supabase.from("whatsapp_messages").select("id, direction, message_type, text_content, status, provider_timestamp, created_at, created_by").eq("org_id", orgId).eq("conversation_id", selectedConversation.id).order("created_at", { ascending: false }).limit(200);
    messages = response.data ?? [];

    // Mostra quem da equipe mandou cada mensagem enviada pelo Kavro — sem
    // isso não dá pra saber qual atendente falou com o lead.
    const senderIds = Array.from(new Set(messages.map((message) => message.created_by).filter((id): id is string => Boolean(id))));
    if (senderIds.length) {
      const { data: profiles } = await supabase.from("user_profiles").select("id, full_name, email").in("id", senderIds);
      for (const profile of profiles ?? []) senderNames[profile.id] = profile.full_name || profile.email || "Equipe";
    }
  }
